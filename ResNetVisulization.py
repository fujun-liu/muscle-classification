...
class MuscleModelPredict():
    ...
    def _inception_feat_extractor(self, x):
        return (nn.Sequential(
            # 299 x 299 x 3
    ...
    def init_test(self, model_path, arch, test_transform=None, batch_sz=1, cuda_id=-1):
        self.cuda_id = cuda_id
        self.batch_sz = batch_sz
        self.test_transform = test_transform
        if self.cuda_id > -1:
    ...
    def extract(self, img_lst):
        ...
        for b in range(num_batches):
            ...
            Feat = feat_this is Feat is None else np.concatenate((Feat, feat_this), axis=0)
        return Feat
