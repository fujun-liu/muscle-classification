...
class MuscleModel():
    ...
    def init_train(self, num_classes, init_lr=1e-3, momentum=0.90, lr_decay_epoch=10, l1_penalty=.0,
    ...
    def _build_model(self, num_classes, init_lr, momentum, weight_decay):
        if arch == 'vgg':
            ...
            model_ft.avgpool = nn.Sequential(nn.UpsamplingBilinear2d(scale_factor=self.scale_factor), 
                        nn.Conv2d(512, 128, kernel_size=3, stride=1, padding=1, bias=False),nn.AvgPool2d(7*self.scale_factor))
            model_ft.fc = nn.Linear(128, num_classes, bias=True)
        elif arch == 'resnet34':
        ...
        for phase in ['train', 'val']:
            ...
            for data in self.dset_loaders[phase]:
                ...
                if self.l1_penalty > .0:
                    loss += self.l1_penalty * torch.sum(torch.abs(self.model_ft.fc.weight))
